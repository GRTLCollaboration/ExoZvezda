...
void BinaryEqualMassFix::compute_1d_solution(const double max_r)
{
    ...
    catch (std::exception &exception)
    {
        MayDay::Error("Yikes! I cannot compute the 1d solution in "
                      "BinaryEqualMassFix.impl.hpp file.");
    }
}
...
void BinaryEqualMassFix::compute(Cell<data_t> current_cell) const
{
    ...
    BosonStarHelperFunction::BS_3d_vars star1_vars = helper.compute_star_vars(
        coords, rapidity, -separation / 2, impact_parameter / 2, m_1d_sol,
        phase_offset, false);
    BosonStarHelperFunction::BS_3d_vars star2_vars = helper.compute_star_vars(
        coords, -rapidity2, separation / 2, -impact_parameter / 2, m_1d_sol2,
        0.0, antiboson);
    ...
    // We calculate the effect of object 1 on object 2. This will represents the
    // value to be substracted in the initial data from the position of
    // object 2. This effect is the same for q=1 binary.
    ...
    {
        gammaLL[i][j] =
            star1_vars.gLL[i][j] + star2_vars.gLL[i][j] - helferLL[i][j];
    }
