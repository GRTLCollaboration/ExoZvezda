...
public:
    ...
    {
        double lapse; // lapse
        double shiftx;  // x-component of shift (all others are zero for a spherically symmetric BS)
    ...
    BS_3d_vars compute_star_vars(const Coordinates<double>& coords, double rapidity, double separation_offset, double impact_offset, const BosonStarSolver& sol, double phase_offset, bool is_antiboson) const
    {
        // Initialise
        BS_3d_vars vars;
        vars.lapse = 0.0;
        ...
        double omega_prime_ = sol.get_dlapse_interp(r); // derivative of the lapse
        double psi_ = sol.get_psi_interp(r); // conformal factor
        double psi_prime_ = sol.get_dpsi_interp(r); // derivative of the conformal factor
        double pc_os = psi_ * psi_ * c_ * c_ - omega_ * omega_ * s_ * s_;

        // BS frequency
        double w_ = is_antiboson ? -sol.get_BSfrequency() : sol.get_BSfrequency();
        ...
        vars.Pi_Re += -(1. / lapse) * ((x / r) * (s_ - beta_x * c_) * dp_ * cos(phase_) - w_ * (c_ - beta_x * s_) * p_ * sin(phase_));
        vars.Pi_Im += -(1. / lapse) * ((x / r) * (s_ - beta_x * c_) * dp_ * sin(phase_) + w_ * (c_ - beta_x * s_) * p_ * cos(phase_));
        ...
        vars.KLL[0][1] = lapse * c_ * s_ * (y / r) * (psi_prime_ / psi_ - omega_prime_ / omega_);
        vars.KLL[0][2] = lapse * c_ * s_ * (z / r) * (psi_prime_ / psi_ - omega_prime_ / omega_);
        vars.KLL[1][0] = vars.KLL[0][1];
        vars.KLL[2][0] = vars.KLL[0][2];
        vars.KLL[2][1] = 0.;
        vars.KLL[1][2] = 0.;
        vars.KLL[0][0] = lapse * (x / r) * s_ * c_ * c_ * (psi_prime_ / psi_ - 2. * omega_prime_ / omega_ + v_ * v_ * omega_ * omega_prime_ * pow(psi_, -2));
